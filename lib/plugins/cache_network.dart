...
class CacheNetWorkUtils {
  ...
  /// 图片缓存
  static ImageProvider? getNetworkImageProvider(String? avatar, {String? siteKey}) {
    try {
      if (avatar == null || avatar == "") {
        return null;
      }
      return ExtendedNetworkImageProvider(
        proxyImageUrl(avatar, siteKey: siteKey),
        cache: true,
        retries: 3,
  ...
  static CircleAvatar getCircleAvatar2(
    ...
    return CircleAvatar(
      foregroundImage: avatar.isNotNullOrEmpty ? CacheNetWorkUtils.getNetworkImageProvider(avatar, siteKey: siteKey) : null,
      radius: radius,
  ...
  static Widget getCircleAvatar(
    ...
    return CircleAvatar(radius: radius, child: getCacheImageV2(avatar, radius: radius, fit: BoxFit.fitWidth, shape: BoxShape.circle, cacheWidth: 60, cacheHeight: 60, siteKey: siteKey));
  }
  ...
  static String proxyImageUrl(
    ...
    if (siteKey.isNotNullOrEmpty && SettingsService.instance.networkImageProxyExcludeList.contains(siteKey)) return imageUrl;
