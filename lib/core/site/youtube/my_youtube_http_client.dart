...
import 'package:youtube_explode_dart/src/reverse_engineering/hls_manifest.dart';
...
class MyYoutubeHttpClient extends YoutubeHttpClient {
  ...
  MyYoutubeHttpClient([HttpClient? httpClient]) : _httpClient = httpClient ?? HttpClient();
  ...
  http.Response toHttpResponse(dio.Response response) {
    return http.Response(response.data, response.statusCode ?? 200);
  }
  ...
  Future<http.Response> get(
    ...
    final response = toHttpResponse(await _httpClient.get(url.toString(), header: headers));
    if (_closed) throw HttpClientClosedException();
  ...
  Future<http.Response> post(
    ...
    final response = toHttpResponse(await _httpClient.post(url.toString(), header: headers, data: body));
    if (_closed) throw HttpClientClosedException();
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    ...
    var result = await _httpClient.request(request.url.toString(), data: data, options: dio.Options(method: request.method, headers: request.headers));
    var rsData = result.data.toString();
    final bytes = utf8.encode(rsData);
    var byteStream = http.ByteStream.fromBytes(bytes);
    return http.StreamedResponse(byteStream, result.statusCode ?? 200);
  }
}
