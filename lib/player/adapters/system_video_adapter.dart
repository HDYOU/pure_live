...
/// 系统 VideoPlayer + Chewie 适配器
/// 使用 Flutter 官方 video_player 和 chewie 播放器
class SystemVideoAdapter implements UnifiedPlayer {
  ...
  Future<void> init({bool audioOnly = false}) async {
    ...
    try {
      ...
      final exception = PlayerException(
        message: 'System VideoPlayer init failed',
  ...
  Future<void> setDataSource(
    ...
    try {
      ...
      final exception = PlayerException(
        message: 'System VideoPlayer setDataSource failed',
  ...
  void _bindVideoListener() {
    ...
    _videoListener = () {
      ...
      if (value.hasError) {
        final exception = PlayerException(
          message: value.errorDescription ?? 'VideoPlayer error',
